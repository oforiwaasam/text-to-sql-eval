...
error_mask = df['Generated_SQL'] == 'ERROR'
error_counts = df[error_mask].groupby('Model_Name').size()
df = df[~error_mask]
...
# Store results for the console leaderboard and CSV export
results_summary = []
...
for idx, model in enumerate(models):
    ...
    n_errors = int(error_counts.get(model, 0))
...
# Save leaderboard to CSV so these numbers don't have to be regenerated
summary_df = pd.DataFrame(results_summary)
summary_df.to_csv('model_leaderboard.csv', index=False)
print("✅ Leaderboard saved as 'model_leaderboard.csv'\n")
 
print("-" * 80)
print(f"{'Model Name':<40} | {'Acc':<5} | {'Conf':<5} | {'ECE':<5} | {'Errors':<6}")
print("-" * 80)
for res in results_summary:
    print(f"{res['Model']:<40} | {res['Accuracy']:.3f} | {res['Confidence']:.3f} | {res['ECE']:.3f} | {res['N_Errors']:<6}")
print("-" * 80)